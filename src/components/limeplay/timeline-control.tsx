...
>((props, ref) => {
  ...
  const { className, onValueChange, orientation = "horizontal", ...etc } = props
  ...
  >((value, eventDetails) => {
    ...
    seek(timelineValueToTime(nextValue, duration, isLive, seekRange))
  }, [duration, isLive, onValueChange, player, seek])
...
export function timelineValueToTime(
  value: number,
  duration: number,
  isLive: boolean,
  ...
  return isLive
    ? seekRange.start + progress * (seekRange.end - seekRange.start)
    : progress * duration
}
